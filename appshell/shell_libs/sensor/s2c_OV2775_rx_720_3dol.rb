...
csi_i2c_write(0x6c, 0x3013, 0x01)

puts "i2c_cfg( wr, bus: 0x6c, 0x3013, 0x01), then no_raw." 
delay_time = 10  * 0.001

puts "delay( %s seconds)." % [ delay_time]

#time = Time.new
#puts time.inspect
#puts time.usec
sleep(delay_time.to_f)
#time = Time.now
#puts  time.inspect
#puts time.usec

[
  ###### MIPI HCG+LCG+VS  720P10  @HCG
    [ 0x6c, 0x3000, 0x02],
    [ 0x6c, 0x3001, 0x28],
    [ 0x6c, 0x3002, 0x03],
    [ 0x6c, 0x3003, 0x01],
  ...
  if(i2c_reg_item[1] == 0x3013)
	  puts "i2c_cfg( wr, bus: %01x, dev=0x%02x, reg_idx=0x%02x, wdata=0x%02x), then no_raw." % [bus_id, i2c_reg_item[0], i2c_reg_item[1], i2c_reg_item[2]]
  else
