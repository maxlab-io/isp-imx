...
def nwl_bus_init_settings(bus_id, is_init):
    ...
    if bus_id == 0:
        handle = subprocess.Popen("./s2c_OV2775_rx_720_3dol.rb %s"%(hex(bus_id)), stdout=subprocess.PIPE, shell=True)
    elif bus_id == 1:
        handle = subprocess.Popen("./s2c_OV2775_rx_720_3dol.rb %s"%(hex(bus_id)), stdout=subprocess.PIPE, shell=True)
    else:
        print "invalid input %d"%bus_id 
...
def nwl_bus_init_settings_nat(bus_id, is_init):
    print "bus input %d"%bus_id 
