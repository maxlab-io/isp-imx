...
ISP_IMX_BRANCH="integration_20200827_4.2.2_p2_basler_v2.0.1"

ISP_VVCAM_GIT="ssh://git@bitbucket.sw.nxp.com/imxpriv/verisilicon_sw_isp_vvcam.git"
# https://bitbucket.sw.nxp.com/projects/IMXPRIV/repos/verisilicon_sw_isp_vvcam
ISP_VVCAM_BRANCH="imx_5.4.47_2.2.0"
...
BASLER_CAMERA_BRANCH="integration_daA3840_30mc_NXP_4.2.2_patch2_20200827-imx8mp_v2.0.1-bundle"
...
git_checkout_branch () {
	...
	if [ $GIT_BRANCH = $CURRENT_BRANCH ]; then
		...
		git checkout $GIT_BRANCH || git checkout -b $GIT_BRANCH -t origin/$GIT_BRANCH || exit 1
		git reset --hard "origin/$GIT_BRANCH" || exit 1
	fi
...
./build-all-isp.sh $BUILD_TYPE full
echo "ISP libs build status: $?"
BUILD_OUTPUT_BASENAME="build_output_${BUILD_TYPE}_full"
