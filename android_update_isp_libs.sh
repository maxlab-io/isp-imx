...
{
    if [ -e "$DST_PWD/isp/config/daA3840_30mc_1080P.json" ];then
        rm $DST_PWD/isp/config/daA3840_30mc_1080P.json
    fi

    CONFIG_DIR=dewarp/dewarp_config
    if [ -e "$CONFIG_DIR/$1" ];then
        mkdir -p $DST_PWD/isp/config/dewarp_config && \
        cp $CONFIG_DIR/$1   $DST_PWD/isp/config/dewarp_config/$1 && \
        echo "update the file $DST_PWD/isp/config/dewarp_config/$1 based on $CONFIG_DIR/$1"
    else
...
function update_xml()
{
    CONFIG_DIR=$SRC_PWD/bin
    if [ -e "$CONFIG_DIR/$1" ];then
        cp $CONFIG_DIR/$1   $DST_PWD/isp/config/$1 && \
        echo "update the file $DST_PWD/isp/config/$1 based on $CONFIG_DIR/$1"
    else
        echo "$CONFIG_DIR/$1 not exits!"
    fi
}
...
function update_all()
{
    ...
    update_bin vvext
    update_json daA3840_30mc_1080P.json
    update_json daA3840_30mc_4K.json
    update_xml DAA3840_30MC_4K.xml
    update_xml DAA3840_30MC_1080P.xml
}
