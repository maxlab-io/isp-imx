        ...
        if (af.isEnable) {
            ...
            if (config.mode != af.config.mode 
                    || (config.mode == CAM_ENGINE_AF_MODE_AUTO 
                        && (config.isOneshot != af.config.isOneshot 
                            || config.searchAlgorithm != af.config.searchAlgorithm))) {
...
int32_t Engine::afEnableSet(bool isEnable) {
    ...
    if (isEnable) {
        ...
        if (af.config.isOneshot  && af.config.mode == CAM_ENGINE_AF_MODE_AUTO) {
            int32_t ret = CamEngineAfOneShot(hCamEngine, af.config.searchAlgorithm, af.config.mode);
            REPORT(ret);
        } else {
            af.config.isOneshot = false;
            int32_t ret = CamEngineAfStart(hCamEngine, af.config.searchAlgorithm, af.config.mode);
            REPORT(ret);
        }
