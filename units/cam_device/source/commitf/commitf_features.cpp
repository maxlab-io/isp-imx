...
RESULT CitfDevice::calibrationLoad(Json::Value &jRequest, Json::Value &jResponse) {
    ...
    try {
        ...
        TRACE(CITF_ERR, "%s error(L%d): load calibration %s failed!\n", __func__, __LINE__, fileName.c_str());
...
RESULT CitfDevice::cameraConnect(Json::Value &jRequest, Json::Value &jResponse) {
    ...
    RESULT ret = pCitfHandle->pOperation->cameraConnect(preview, pBufferCb, ConnectFormat);
    return ret;
}
...
RESULT CitfLsc::statusGet(Json::Value &jRequest, Json::Value &jResponse) {
    ...
    count = sizeof(status.config.gain.LscRDataTbl) / sizeof(status.config.gain.LscRDataTbl[0]);
    for (int32_t i = 0; i < count; i++) {
        jMap[LSC_RED_PARAMS].append(LSC_UINT16_2_FLOAT(status.config.gain.LscRDataTbl[i]));
        jMap[LSC_GREEN_R_PARAMS].append(LSC_UINT16_2_FLOAT(status.config.gain.LscGRDataTbl[i]));
        jMap[LSC_GREEN_B_PARAMS].append(LSC_UINT16_2_FLOAT(status.config.gain.LscGBDataTbl[i]));
        jMap[LSC_BLUE_PARAMS].append(LSC_UINT16_2_FLOAT(status.config.gain.LscBDataTbl[i]));
    }

    count = sizeof(status.config.grid.LscXSizeTbl) / sizeof(status.config.grid.LscXSizeTbl[0]);
    for(int32_t i = 0; i < count * 2; i++) {
        if (i < 8) {
            jMap[LSC_XSIZE_PARAMS].append(status.config.grid.LscXSizeTbl[i]);
            jMap[LSC_YSIZE_PARAMS].append(status.config.grid.LscYSizeTbl[i]);
        } else {
            jMap[LSC_XSIZE_PARAMS].append(status.config.grid.LscXSizeTbl[count - 1 - i % count]);
            jMap[LSC_YSIZE_PARAMS].append(status.config.grid.LscYSizeTbl[count - 1 - i % count]);
        }
    }

    jResponse[LSC_MAP_PARAMS] = jMap;
