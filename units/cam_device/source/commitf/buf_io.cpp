...
RESULT Xim::start(void *) {
    ...
    if (state == Running) {
        TRACE(CITF_ERR, "%s Wrong status %d (exit)\n", __PRETTY_FUNCTION__, state);
...
RESULT Xom::xomInit(uint8_t skip_interval){
    ...
    {
            TRACE(CITF_ERR, "%s (creating full buffer queue (depth: %d) failed)\n", __func__, MaxBuffers);
...
RESULT Xom::xomDeInit(){
    ...
    if(state != Idle)
    {
        TRACE(CITF_ERR, "%s (Invalid Status %d)\n", __func__, state);
        return RET_WRONG_STATE;
    }
...
void Xom::bufferCb(MediaBuffer_t *pBuffer) {
    ...
    if(state != Running)
    {
        TRACE(CITF_ERR, "%s (Wrong Xom state %d)\n", __func__, state);
        return;
    }
    ...
    if(ret != RET_SUCCESS) {
        TRACE(CITF_ERR, "%s (Send command error, ret is %d)\n", __func__, ret);
    }
}
...
RESULT Bitf::bitfSetOutputCtrl(ISPCORE_BUFIO_ID id, Xom * pXom){
    ...
    {
        TRACE(CITF_ERR, " %s Wrong ID channel %d, large than %d\n", __PRETTY_FUNCTION__, (int)id, (int)ISPCORE_BUFIO_WRITEMAX - 1);
...
RESULT Bitf::bitfSetInputCtrl(Xim * pXim){
    ...
    if(pXim != nullptr)
    {
        ...
        {
            TRACE(CITF_ERR, " %s Initialize BuffPool firstly, current bufio ctrl state:%d\n", __PRETTY_FUNCTION__, bufio_ctrl[ISPCORE_BUFIO_READ].state);
            return RET_WRONG_STATE;
        }
    }
...
RESULT Bitf::bitfSetBufferPoolCtrl(ISPCORE_BUFIO_ID id, BuffPool * pBufPool){
    ...
    if((id < ISPCORE_BUFIO_WRITEMAX) ||(id == ISPCORE_BUFIO_READ))
    {
        ...
        TRACE(CITF_ERR, " %s Wrong ID channel %d\n", __PRETTY_FUNCTION__, (int)id);
        return RET_INVALID_PARM;
    }
    return RET_SUCCESS;
}
